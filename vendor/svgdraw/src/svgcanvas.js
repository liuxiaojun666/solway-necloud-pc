...
this.cloneSelectedElements = function(x,y, drag) {
  ...
  while (i--) {
    ...
    var type = clone.getAttribute("type");
    if (type && window.ExtShape && ($.inArray(type, window.ExtShape))){
      clone.setAttribute("deviceId", "");
      svgCanvas.runExtensions("callBackSelect", {"elem": clone, "type": type}, true);
    }
  }
...
var changeSelectLi = this.changeSelectLi = function (type) {
  var elems = svgCanvas.getSelectedElems();
  if (elems == null || elems.length > 1) return;
  $("#"+elems[0].getAttribute("id")).attr("deviceId", $("div[class='select_input_content'][type='"+type+"']").attr("data-id"));
};
