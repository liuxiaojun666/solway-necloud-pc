...
(function() {
  ...
  if(!window.methodDraw) window.methodDraw = function($) {
    ...
    curConfig = {
      canvas_expansion: 1, 
      dimensions: [580,400], 
      initFill: {color: 'F0F3F4', opacity: 1},
      initStroke: {width: 1.5, color: 'F0F3F4', opacity: 1},
      initOpacity: 1,
      imgPath: 'images/',
      extPath: 'extensions/',
        ...
        if (elem != null) {
          ...
          if (el_name == "path" || el_name == "polyline") {
            ...
            if (elems != null && elems.length == 1){
              var title = elem.getAttribute("type");
              $("#"+title+"_panel").show();
              svgCanvas.runExtensions(
                  "call"+title.replace(title.charAt(0), title.charAt(0).toUpperCase()),
                  {"elem": elem}, true);
            }
          }
          ...
          if (el_name == "g"){
            ...
            if (elems != null && elems.length == 1){
              ...
              if (title != null && title != ""){
                $("#"+title+"_panel").show();
                svgCanvas.runExtensions("callBackSelect", {"elem": elem, "type": title}, true);
              }
            }
          }
...
//        var opacity = (picker == 'stroke' ? $('#stroke_opacity') : $('#fill_opacity'));
        ...
        if (paint.type === 'pattern') {
            paint = new $.jGraduate.Paint({alpha: 100, type: "solidColor", solidColor: curConfig['initFill'].color});
        }
      ...
      var PaintBox = function(container, type) {
        ...
        var cur = {color: "F0F3F4", opacity: 1}
        ...
        this.setPaint = function(paint, apply, noUndo) {
          ...
          switch ( ptype ) {
            ...
            case 'radialGradient':
              ...
              break;
            case 'pattern':
              //fillAttr = "url(#" + 'imageBackground' + ')';
              fillAttr = "url(#" + paint.pattern.id + ')';
          }
          if (this.paint.type == 'pattern') {
            this.rect.setAttribute('fill', '#' + curConfig['initFill'].color);
          } else {
    ...
    var changeCanvasSize = function(ctl){
      ...
      var Actions = function() {
        ...
        var tool_buttons = [
          ...
          {sel:'#tool_saveData', fn: function() { window.parent.saveSvgDataToDataBase(); }, evt: 'mouseup'},
          ...
          {key: [modKey + 'V', true], fn: function(){svgCanvas.cloneSelectedElements(0,-1)}},
