(function ($) {
    ...
    function initWebUpload(element, options) {
        ...
        var defaults = {
            auto:true,
            url: '/fileUpload',
            deleteUrl:'/fileUpload/delete',
            deleteByField: 'id',
            deleteServerFlag: true,
            files: [],
        ...
        uploader.on('fileDequeued', function (file) {
            if(opts.deleteServerFlag) {
                var fieldVal = $("#hiddenInput" + $uploader.attr('id') + file.id + '_' + opts.deleteByField).val();
                if (fieldVal!=null) {
                    $.post(opts.deleteUrl, { deleteField: fieldVal }, function (data) {
                        //alert(data.message);
                        //console.info(data);
                    })
                }
        ...
        $list.on("click", ".webuploadDownloadbtn", function () {
            var $ele = $(this).parent();
            var obj = {};
            $('#hiddenInput' + $ele.attr("id")).find('input').each(function () {
