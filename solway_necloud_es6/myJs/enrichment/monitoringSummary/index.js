ajaxData({
...
}, {})('monitoringSummaryCtrl', ['$scope', 'myAjaxData', 'actionRecord'], ($scope, myAjaxData, historicalRecord) => {
    historicalRecord.init();
    $scope.beforeLoading = true; // 页面loading
    $scope.moduleName = '电站监视';//当前页面名称；
    historyInitCallback();
    //主题功率传来的更新时间和状态
    $scope.$on('lastUpdate',(i,val)=>{
        $scope.comm = val.comm;
        $scope.dtime = val.dtime;
    });
    ...
    $scope.changeTheme = theme => {
        ...
        if ($scope.themeDetail === `tpl/enrichment/monitoringSummary/components/${folder}/${file}.html`) return;
        $scope.themeLoading = true;
        $scope.themeDetail = `tpl/enrichment/monitoringSummary/components/${folder}/${file}.html`;
    };
    ...
    function keyChangeTheme(event) {
        ...
        if ($scope._dateType > 0) themeArr.splice(2, 1);
        const { theme } = $scope;
