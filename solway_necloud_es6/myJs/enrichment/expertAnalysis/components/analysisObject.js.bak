ajaxData({
    ...
    queryCascaderWithDevices: {
        ...
        $scope.$on('alsObjHdata', () => {
            ...
            if ($scope.analysisObjectK == 1 || $scope.analysisObjectK == 3 || $scope.analysisObjectK == 5) {
                ...
                // setTimeout(()=>{
                    ...
                    $scope.equipChoice(parentmyAjaxData.config.tableData[parentmyAjaxData.config.checkChart].alsObj.equipCItem, parentmyAjaxData.config.tableData[parentmyAjaxData.config.checkChart].alsObj.equipCIndex);
            ...
            if (k == 0) {
                ...
                $scope.zNodes.map((item) => {
                    if (!JSON.parse($scope.multiLevel.area).busiId) {
                        ...
                        // JSON.parse($scope.multiLevel.area).busiType && item.name!='全部'
                        if (item.busiType == JSON.parse($scope.multiLevel.area).busiType && item.name!='全部') {
                            item.isHidden = true;
                        }
                        if (item.busiId == JSON.parse($scope.multiLevel.area).busiId) {
                            item.isHidden = false;
                        }
                    }
                })
        ...
        $scope.radioFault = (isHistory) => {
            $scope.analysisObjectK = 101;
            parentmyAjaxData.config.dmsType = 102;
            $scope.zNodes = [];
            treeData('101');
            $scope.$emit('radioToFather', { 'type': 'analysisObject', 'k': 101, 'value': '故障类型', 'isHistory': isHistory });
            selectArea();
            selectPowerStation();
            selectMachineType();
            selectDevice();
        ...
        function treeData(k) {
            ...
            if (k == 1 || k == 3 || k == 5) {
                ...
                $scope.queryStTreeWithFaultTypes.subscribe(res => {
                    $scope.zNodes = res.body;
                    $.fn.zTree.init($("#tree"), $scope.setting, $scope.zNodes);
                    $scope[key[$scope.analysisObjectK]].map((item) => {
                        item.map((son) => {
        ...
        $scope.oneRItem5 = {};    //区域
        $scope.oneRItem3 = {};    //集团
        $scope.oneRItem100 = [];  //机型
        $scope.oneRItem101 = [];  //故障类型
        ...
        $scope.oneAdd = () => {
            ...
            //把过滤后的 数据放到 对应的数组中
            if ($scope.analysisObjectK == 100 || $scope.analysisObjectK == 101) {
                //机型/故障类型  获取根节点
                var typeArr = [];
            ...
            } else if ($scope.analysisObjectK == 100 || $scope.analysisObjectK == 101) {
                //组装接口需要的数据 (机型、故障类型)
                $scope['oneRItem' + $scope.analysisObjectK] = [];
                typeArr.map((item) => {
                    var oneRItem = {
        ...
        $scope.groupResult100 = [];  //机型
        $scope.groupResult101 = [];  //故障类型
            ...
            if ($scope.analysisObjectK == 1 || $scope.analysisObjectK == 3 || $scope.analysisObjectK == 5) {
                ...
                item.map((v) => {
                    if (v.isgroup) {
                        ...
                        var index = $scope.oneRItemDevice.st_eqids.indexOf(`${v.stids}-${v.busiId}`);
                        if (index != -1) {
        ...
        //全部删除
        $scope.delRightAll = () => {
            if (($scope.analysisObjectK == 99 && $scope.objectDevice[$scope.equipChoiceIndex].length == 0) || (($scope.analysisObjectK == 1 || $scope.analysisObjectK == 3 || $scope.analysisObjectK == 5 || $scope.analysisObjectK == 100 || $scope.analysisObjectK == 101) && $scope[key[$scope.analysisObjectK]].length == 0)) {
        ...
        $scope.confirm = () => {
            if ($scope.analysisObjectK == 1 || $scope.analysisObjectK == 3 || $scope.analysisObjectK == 5) {
                if ($scope[key[$scope.analysisObjectK]].length == 0) {
                    toaster.pop('error', '', '所选数据不能为空');
                    return;
                } else if (parentmyAjaxData.config.anlsType == 1 && $scope[key[$scope.analysisObjectK]].length > 10){
                ...
                parentmyAjaxData.config.dmsTypeDs[parentmyAjaxData.config.checkChart] = [];
                var Bs;
                Bs = Object.keys($scope['oneRItem' + $scope.analysisObjectK]).length == 0 ? [...$scope['groupResult' + $scope.analysisObjectK]] : $scope['groupResult' + $scope.analysisObjectK].length == 0 ? [$scope['oneRItem' + $scope.analysisObjectK]] : [...$scope['groupResult' + $scope.analysisObjectK], $scope['oneRItem' + $scope.analysisObjectK]];
                parentmyAjaxData.config.dmsTypeBs[parentmyAjaxData.config.checkChart] = JSON.parse(JSON.stringify(Bs));
            } else if ($scope.analysisObjectK == 99) {
                if ($scope.objectDevice[$scope.equipChoiceIndex].length == 0) {
                    toaster.pop('error', '', '所选数据不能为空');
                    return;
                } else if (parentmyAjaxData.config.anlsType == 1 && $scope.objectDevice[$scope.equipChoiceIndex].length > 10){
                    toaster.pop('error', '', '分析对象不能超过十个');
                    return;
                }
                parentmyAjaxData.config.dmsTypeBs[parentmyAjaxData.config.checkChart] = [];
                ...
                parentmyAjaxData.config.dmsTypeDs[parentmyAjaxData.config.checkChart] = [];
                var Ds;
                Ds = Object.keys($scope.oneRItemDevice).length == 0 ? [...$scope.groupResultDevice] : $scope.groupResultDevice.length == 0 ? [$scope.oneRItemDevice] : [...$scope.groupResultDevice, $scope.oneRItemDevice];
                ...
                parentmyAjaxData.config.dmsTypeAs[parentmyAjaxData.config.checkChart] = [];
                parentmyAjaxData.config.dmsTypeBs[parentmyAjaxData.config.checkChart] = [];
                parentmyAjaxData.config.dmsTypeCs[parentmyAjaxData.config.checkChart] = JSON.parse(JSON.stringify($scope['oneRItem' + $scope.analysisObjectK]));
                parentmyAjaxData.config.dmsTypeDs[parentmyAjaxData.config.checkChart] = JSON.parse(JSON.stringify($scope['groupResult' + $scope.analysisObjectK]));
            }
            //存储历史数据
