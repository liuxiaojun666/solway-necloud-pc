...
HighAnalysis_selectFds: {
    ...
    $scope.HighAnalysis_selectFds.subscribe(res => {
        ...
        else if ($scope.anlsType == 3 && parentmyAjaxData.config.tableDataTrue[$scope.checkChart]&&(parentmyAjaxData.config.anlsType==parentmyAjaxData.config.tableData[$scope.checkChart].analysisType)&&(parentmyAjaxData.config.dmsType==parentmyAjaxData.config.analysObjData[parentmyAjaxData.config.checkChart].dmsType)){
            return;
        }
        else if ($scope.anlsType == 2 && parentmyAjaxData.config.tableDataTrue[$scope.checkChart] &&(parentmyAjaxData.config.anlsType==parentmyAjaxData.config.tableData[$scope.checkChart].analysisType)&&(parentmyAjaxData.config.dmsType==parentmyAjaxData.config.analysObjData[parentmyAjaxData.config.checkChart].dmsType)&&($scope.dmsTime==parentmyAjaxData.config.tableData[$scope.checkChart].timeLat)){
            return;
        } else {
            for(key in $scope.analysData){
                $scope.analysData[key].ll.map((item)=>{
    ...
    $scope.$on('analysOpen', () => {
        ...
        if($scope.analysCheckData.length > 0){
            var key;
            for(key in $scope.analysData){
                $scope.analysData[key].ll.map((son)=>{
                    if(son.fdKey == $scope.analysCheckData[0].fdKey){
                        son.checked = true;
                    }
                })
            }
    ...
    $scope.$on('alsObjHdata', () => {
        if(parentmyAjaxData.config.anlsType != 1){
            ...
            $scope.query.sortType = parentmyAjaxData.config.tableData[parentmyAjaxData.config.checkChart].alys.sortType;
            $scope.query.tableType = parentmyAjaxData.config.tableData[parentmyAjaxData.config.checkChart].alys.tableType;
    ...
    $scope.confirm = () => {
        ...
        parentmyAjaxData.config.benchSort[parentmyAjaxData.config.checkChart] = parentmyAjaxData.config.anlsType==3 ? $scope.analysCheckData[0].fdKey + ',' + $scope.query.sortType : null;
        parentmyAjaxData.config.analysFds[parentmyAjaxData.config.checkChart][0] = analysFds_y1;
        // $scope['analysFdsArr'+ parentmyAjaxData.config.checkChart] = JSON.parse(JSON.stringify(parentmyAjaxData.config.analysFds[parentmyAjaxData.config.checkChart]));

        parentmyAjaxData.config.fdStyle = $scope.query.tableType;
        parentmyAjaxData.config.fdStyleArr[parentmyAjaxData.config.checkChart] = $scope.query.tableType;
        //存储历史数据
        ...
        parentmyAjaxData.config.alysHdata[parentmyAjaxData.config.checkChart].sortType = $scope.query.sortType;
        parentmyAjaxData.config.alysHdata[parentmyAjaxData.config.checkChart].tableType = $scope.query.tableType;
