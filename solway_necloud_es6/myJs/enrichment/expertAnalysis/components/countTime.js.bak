    ...
    AddbaseDictionary: {
        ...
        $scope.confirm = () => {
            if ($scope[key[$scope.radioIndex]].length == 0) {
                ...
                return;
            } else if ($scope.dateType == 3 && $scope.dmsTime == 5 && parentmyAjaxData.config.analysObjData.analysisObjectK == 99 && $scope[key[$scope.radioIndex]].length*parentmyAjaxData.config.analysObjData.checkData[parentmyAjaxData.config.analysObjData.equipChoiceIndex].length >= 31){
