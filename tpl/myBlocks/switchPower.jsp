...
<script type="text/javascript">
	...
	app.controller('switchPowerCtrl', function($scope, $rootScope, $http, $state, $stateParams, $location) {
		...
		$scope.getSTClass = function () {
			$http({
				method : "POST",
				url : "/NECloud/UserAuthHandle/getSTClassLIst.htm",
		...
		$scope.switchStation = function ($event, powerStationId , stFlag , dataName, isGroup) {
			...
			$http({
				method : "POST",
				url : "/NECloud/UserAuthHandle/changeDataType.htm",
				params: {
					id: powerStationId,
					dataType: stFlag - 1,
					isGroup : isGroup,
				...
				if(res.result){
					if(res.isChangeSTClass == 'yes'){
							...
							if(res.isNullSTClass == 'yes'){
								...
								window.location.href="/index.html#/app"+$rootScope.firstMenuUrl[currentSTClassVar]["uiurl"];
								location.reload();
							}
		...
		$scope.getAllSTList = function (stClass) {
			$http({
				method: 'POST',
				url: "/NECloud/UserAuthHandle/getAllSTListBySession.htm",
				params: {
