...
<script type="text/javascript">
		...
		var conti= checkSelectMsg("ids","请至少选择一条记录!");
			...
			if(conti){
				var options = {
						type:"post",
						dataType:"json",
						url : "${ctx}/PaySettlement/deleteBatch.htm?"+$("input[name=ids]:checked").serialize(),
	...
	app.controller('PaySettlementCtrl', function ($scope, $http,$state) {
		...
		$scope.onSelectPage = function(page) {
			...
			$http({
				method : "POST",
				url : "${ctx}/PaySettlement/paySettlementList.htm",
				params : {
					'pageIndex' : page - 1,
 ...
 <!-- 弹出层界面 -->
<div id="productEdit" data-ng-include="'${ctx}/tpl/maintenancePage/paySettlement/addPaySettlement.jsp'"></div>
<div  ng-controller="PaySettlementCtrl" >
	<div class="wrapper-md bg-light b-b">
		...
		<!-- <span class="font-h3 blue-1 m-n text-black">运维支出管理</span> -->
	</div>
	<div class="wrapper-md ng-scope">
      ...
      <div class="col-sm-4 m-b-xs w-sm p-r-sm">
           <button class="btn btn-sm btn-info" onclick="addData('paySettlementModal');"> 新增</button>
       ...
       <div class="input-group">
          <span class="input-group-btn">
            <button class="btn btn-sm btn-default" type="button">业务单号</button>
          </span>
        <input type="text" id="no" class="input-sm form-control"></div>
      </div>
      <div class="col-sm-1">
	    ...
	    <table id="result_table" class="table table-striped b-t b-light">
			<thead>
				<tr>
					...
					<th>支出单号</th>
					<th>支出项目</th>
					<th style="text-align: center;">支出时间</th>
					<th style="text-align: right;">支出金额（元）</th>
					<th>支出人</th>
					<th>操作</th>
				</tr>
			</thead>
			<tbody role="alert" aria-live="polite" aria-relevant="all">
				<tr  ng-repeat="vo in data ">
					...
					<td ng-bind="vo.payno"></td>
					<td ng-bind="vo.paycontent"></td>
					<td style="text-align: center;" ng-bind="vo.paydate | date:'yyyy-MM-dd'"></td>
					<td style="text-align: right;" ng-bind="vo.paymoney | currency:'￥ '"></td>
					<td ng-bind="vo.payPeople"></td>
					<td>
						<a class="text-info">
		         					 <i class="icon-note" ng-click="editData({{ vo.id }},'paySettlementModal');" ></i></a>
