...
<div ng-controller="BuscfgCtrl" class="modal fade" id="addPBuscfg" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	...
	<div class="wrapper-md ng-scope">
		<div class="panel panel-default">
			<div class="wrapper">
				...
				<!-- form 开始 -->
				<form class="m-t-lg form-horizontal ng-pristine ng-valid" action="${ctx}/BusCfg/update.htm" method="post" id="editbusForm" name="editbusForm">
					<input type="hidden" id="idcfg" name="id">
					<input type="hidden" id="pstationidcfg" name="pstationid" >
					<div class="form-group">
						...
						<label class="col-lg-2 control-label">总线名称</label>
						<div class="col-lg-4">
							<input type="text" id="namecfg" maxlength="50" name="name"  class="form-control formData" >
						</div>
						<label class="col-lg-2 control-label">通讯地址</label>
						<div class="col-lg-4">
							<input placeholder="IP:端口号" type="text" id="hostaddrcfg" name="hostaddr" maxlength="30" class="form-control formData">
						</div>
					</div>
					
					
					<div class="form-group">
						<label class="col-lg-2 control-label">总线模式</label>
						<div class="col-lg-4">
						...
						<label class="col-lg-2 control-label">超时时间(ms)</label>
						<div class="col-lg-4">
							<input type="text" id="timeoutcfg" name="timeout" maxlength="6" class="form-control formData">
						</div>
					</div>
					
					
					<div class="form-group">
						<label class="col-lg-2 control-label">总线分类</label>
	         		 	<div class="col-lg-10">
	              		<label class="checkbox-inline i-checks ">
  	...
  	<div class="form-group m-t-sm ">
		<div class="col-lg-12 text-center ">
			...
			<button id="escCancelBtn" type="button"class=" m-l-sm  btn m-b-xs w-xs btn-default" data-dismiss="modal"><strong>取消</strong></button>
		</div>
	</div>
	</div>
   </div>   
	...
	$(function() {
		$("#editbusForm").validate({
			...
			submitHandler : function(form) {
				...
				$('#editbusForm').ajaxSubmit(options);
			}
		});
		...
		if (id != "" && id != null) {
			$.ajax({
				...
				success : function(msg) {
					$("#namecfg").val(msg.name);
					$("#pstationidcfg").val(msg.pstationid);
					$("#hostaddrcfg").val(msg.hostaddr);
					$("#timeoutcfg").val(msg.timeout);
					
					var passive=msg.passive;
					var passivelen=document.editbusForm.passive.length;
					for(var a=0;a<passivelen;a++){
						if(passive==document.editbusForm.passive[a].value){
			                document.editbusForm.passive[a].checked=true
			            }
					}
					
					var type=msg.type;
					var typelen=document.editbusForm.type.length;
					for(var i=0;i<typelen;i++){
						if(type==document.editbusForm.type[i].value){
			                document.editbusForm.type[i].checked=true
			            }	
					}
					getStationList();
				}
...
app.controller('BuscfgCtrl',['$http','$scope','$stateParams','$state',
							function($http, $scope, $stateParams, $state) {
								$scope.stationd = {};
								$scope.station = null;
								$scope.factd = {};
								$scope.fact = null;
								$("#pstationidcfg").val($scope.pstationid);
