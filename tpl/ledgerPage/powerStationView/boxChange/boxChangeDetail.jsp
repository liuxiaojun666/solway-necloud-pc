...
<div ng-controller="JunctionBoxMonitorCtrl">
	<div class="wrapper-md bg-light b-b">
		<span class="font-h3 blue-1 m-n text-black" ng-bind="name"></span>
	     ...
	     <span class="pull-left" id="mainparameter" ng-bind="mainparameter"></span>
	     <br>
	     <br>
...
<!-- 	     <span class="pull-left"  id="repaircard" ng-bind="repaircard"></span> -->
		...
		<div class="wrapper-sm  no-border-xs">
				...
				<span id="deviceInfo" class="white-1 a-cur-poi" ng-click="onSelectPage(1)">设备信息</span>
				...
				<div class="col-sm-12 wrapper-md b-a panel border-none">
				   <div style="width: 98%;margin: 0 auto;height: 25px;line-height: 25px">
				     ...
				     <div align="left"><font class="font-h3 black-2" style="line-height: 25px" >A相温度</font>&nbsp;&nbsp;&nbsp;
			...
			<table id="result_table" class="table table-striped b-light table-boder" >
				...
				<tab ng-controller="boxChangepageHistory"> 
					<tab-heading class="wrapper-sm">
						<span id="historyInfo" class="white-1 a-cur-poi"  ng-click="onSelectPage(1)">历史信息</span>
	...
	<div class="col-sm-12 pull-right">
		...
		<div class="col-sm-5 pull-right"><%@ include file="/common/pager.jsp"%></div>
				 		...
				 		<div class="col-sm-12 no-padder " id="deviceDatas">
							...
							<div class="col-sm-12 no-padder" ng-grid="boxChangeGridOptions" id="box_change_result_table" style="height: 350px;" ></div>
						</div>
						</paging>
				</tab>
				...
				<tab ng-controller="faultAlarm"> 
					<tab-heading class="wrapper-sm">
						<span id="faultalarmInfo" class="white-1 a-cur-poi" ng-click="onSelectPage(1)">故障信息</span>
					...
					<tab ng-controller="repairt"> 
						<tab-heading class="wrapper-sm">
							<span id="repairtInfo" class="white-1 a-cur-poi" ng-click="onSelectPage(1)">检修维护</span>
					...
					<tab ng-controller="experiment"> 
						<tab-heading class="wrapper-sm">
							<span id="experimentInfo" class="white-1 a-cur-poi" ng-click="onSelectPage(1)">技术监督</span>
	        	  ...
	        	  <tbody>
					...
					<tab ng-controller="replaced"> 
						<tab-heading class="wrapper-sm">
							<span id="replacedInfo" class="white-1 a-cur-poi" ng-click="onSelectPage(1)">技术改造</span>
...
$("#box_change_result_table").css("height", $(window).height() - 425);
...
app.controller('JunctionBoxMonitorCtrl', function($scope, $http, $state,$stateParams) {
	...
	$scope.onSelectPage = function(page) {
		...
		$http({
			...
			$scope.name=result.bboxChange.name;
...
// 			}
		...
		$scope.$on('deviceStation_parameter', function(event,deviceById,tag) {
			alert("我没来"+"tag"+tag+"---"+deviceById.id+"---"+deviceById.deviceSerialNnumber+"---"+deviceById.pstationid);
			if(tag==1){
			...
			angular.element("#deviceInfo").trigger("click");
			}
		});
	};
...
// 			}
		...
		$scope.$on('deviceStation_parameter', function(event,deviceById,tag) {
			...
			getHours($scope, $stateParams,$http);
			$scope.onSelectPage(1);
			angular.element("#historyInfo").trigger("click");
			}
		});
	});
...
function getRtData($scope, $stateParams,$http){
	...
	$http.get("${ctx}/DeviceStation/getSingleRealDataBBoxChange.htm", {
		...
		if(result.respStatus==1){
			...
			$scope.millisecond=result.data.utime;
		}
	}).error(function(response) {
// 		if(response==null){
