...
function getRtData($scope, $stateParams,$http){
	...
	$http.get("${ctx}/DeviceStation/getSingleRealDataWeather.htm", {
		params : {
			id : $stateParams.InId,
		...
		if(result.respStatus==1){
			$scope.serialnumber = result.did;
			$scope.pstationid =result.stid;
			$scope.lh=result.lh;
			$scope.ls=result.ls;
			$scope.la=result.la;
			$scope.lhd=result.lhd;
			$scope.lsd=result.lsd;
			$scope.lad=result.lad;
			$scope.tm=result.tm;
			$scope.ta=result.ta;
			$scope.ws=result.ws;
			$scope.wd=result.wd;
			$scope.h2o=result.h2o;
			$scope.p=result.p;
			$scope.millisecond=result.millisecond;
		}
	}).error(function(response) {
	});
}
