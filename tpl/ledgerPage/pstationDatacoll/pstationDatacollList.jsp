...
<script type="text/javascript">
	...
	function deleteBatch() {
		...
		if (conti) {
			var options = {
				type : "post",
				dataType : "json",
				url : "${ctx}/BPDatacollCfg/deleteBatch.htm?"
	...
	function singledel(id) {
		$.ajax({
			type : "post",
			url : "${ctx}/BPDatacollCfg/delete.htm",
			data : {
				"id" : id
			},
			dataType : "json",
	...
	app.controller('pstationDatacollCtrl', function($scope, $http, $state) {
		...
		$scope.onSelectPage = function(page) {
			...
			$http({
				method : "POST",
				url : "${ctx}/BPDatacollCfg/BPstationDataCfgList.htm",
				params : {
					'pageIndex' : page - 1,
					'pageSize' : $scope.pageSizeSelect,
					'pstationname' : $scope.pstationname,
					'sysVersion' : $scope.sysVersion,
 ...
 <!-- 弹出层界面 -->     
<div id="productEdit" data-ng-include="'${ctx}/tpl/ledgerPage/pstationDatacoll/addpstationDatacoll.jsp'"></div>

<div ng-controller="pstationDatacollCtrl">
	<div class="wrapper-md bg-light b-b">
		<span class="font-h4 blue-1 m-n text-black">电站软件版本配置</span>
	</div>
	<div class="wrapper-md ng-scope">
      ...
      <div class="col-sm-4 m-b-xs w-sm p-r-sm">     
          <button class="btn btn-sm btn-info m-r-xs" onclick="addData('productModal');"> 新增</button>   
      ...
      <div class="col-sm-3 no-padder">
       ...
       <div class="input-group">
        ...
        <input type="text" id="pstationname"  ng-model="pstationname"  class="input-sm form-control">
        </div>
      </div>
       <div class="col-sm-2">
       <div class="input-group">
          <span class="input-group-btn">
            <button class="btn btn-sm btn-default" type="button">系统版本</button>
          </span>
        <input type="text" id="sysVersion" ng-model="sysVersion"  class="input-sm form-control"></div>
      </div>
      <div class="col-sm-1">
	...
	<tbody >
			<tr ng-repeat="vo in data ">
				...
				<td ng-bind="vo.sysVersion"></td>
				<td ng-bind="vo.collectAddr"></td>
				<td ng-bind="vo.managerAddr"></td>
				<td>
					<a class="text-info"><i class="icon-note" ng-click="editData({{ vo.id }},'productModal');" ></i></a>
