...
<!-- 	     <span class="pull-left"  id="repaircard" ng-bind="repaircard"></span> -->
		...
		<div class="wrapper-sm col-sm-12 no-border-xs" >
				...
				<tab ng-click="onSelectPage(1)" class="judgeTabClassBC"> 
			...
			<table id="result_table_BCDetail" class="table table-striped b-light table-boder" >
					<thead>
					<tr>
					<th></th>
					<th>有功功率(kW)</th>
				...
				<tab ng-controller="boxChangepageHistory" active="defaultTabBC" class="judgeTabClassBC" ng-click="changeViewNotHistorySearchbc()"> 
					<tab-heading class="wrapper-sm" >
						<span id="historyInfobc22" class="white-1 a-cur-poi"  >历史数据</span>
						...
						<div class="col-sm-12 no-padder m-t">
								<div class="col-sm-6">
									<div class="input-group " style="width: 400px;">
										<input type="text" id="begindatePublicBC" name="begindatePublicBC" ng-model="begindatePublicBC" class="form-control">
						         	<span class="input-group-addon">至</span>
						         		<input type="text" id="enddatePublicBC" name="enddatePublicBC"  ng-model="enddatePublicBC" class="form-control">
				...
				<tab ng-controller="faultAlarmBC" ng-click="changeViewNotSearchFault()" class="judgeTabClassBC"> 
					<tab-heading class="wrapper-sm">
					...
					<tab ng-controller="repairtBC" ng-click="onSelectPage(1)" class="judgeTabClassBC"> 
						<tab-heading class="wrapper-sm">
					...
					<tab ng-controller="experimentBC" ng-click="onSelectPage(1)" class="judgeTabClassBC"> 
						<tab-heading class="wrapper-sm">
	        	  ...
	        	  <tbody>
					...
					<tab ng-controller="replacedBC" ng-click="onSelectPage(1)" class="judgeTabClassBC"> 
						<tab-heading class="wrapper-sm">
...
//$("#box_change_result_table").css("width", $("#deviceDatas").width);
var bcTimer;
var bcTimer2;
app.controller('JunctionBoxMonitorCtrl', function($scope, $http, $state,$stateParams) {
	...
	$scope.$on('deviceStation_parameter', function(event,deviceById,tag) {
		$("#box_change_result_table").css("height", $(window).height() - 230);
			...
			getHoursBC($scope, $stateParams,$http);
			$scope.getstaDayData();
	...
	$scope.$on('showDeviceDetail3broad', function(event,deviceById) {
        ...
        bcfirstFlagForHS = 0;
	    if(bcfirstFlag++ == 0 && deviceById.defaultShowPage == 2){
	    	 //如果默认显示历史数据页面
	    	 $scope.defaultTabBC = true;
	    	 $scope.$broadcast('toTabsBC',2);
	     }else{
	    	 $(".judgeTabClassBC").each(function(index){
	         	tabNum++;
	         	if($(this).hasClass("active")){
	         		$scope.$broadcast('toTabsBC',tabNum);
	         	}
	...
	bcTimer=setInterval(function(){
		if($scope.deviceTypeNow == '3'){
			getRtDataBC($scope, $stateParams,$http);
			$scope.getstaDayData();
		}
		
	},5000);
	/**bcTimer2=setInterval(function(){
...
// 			}
	...
	app.controller('boxChangepageHistory', function($scope, $http, $state,$stateParams) {
		...
		$scope.onSelectPage  = function(page) {
			...
			//$("#begindatePublicBC").val(startDate(new Date()));
			//$("#enddatePublicBC").val(endDate(new Date()));
			if(flag==0){
				$('#begindatePublicBC').datetimepicker({
					...
					$("#enddatePublicBC").val($('#begindatePublicBC').val());
				    $('#enddatePublicBC').datetimepicker('setStartDate', $('#begindatePublicBC').val()).datetimepicker('update').datetimepicker('show');
				});	
				$('#enddatePublicBC').datetimepicker({
					format: "yyyy-mm-dd hh:ii",
					language: 'zh-CN',
					todayHighlight:true,
					todayBtn:true,
				...
				if($stateParams.dstarttime == null || $stateParams.dstarttime == ""){
					$("#begindatePublicBC").val(startDate(new Date()- 5*60*1000));
					$("#enddatePublicBC").val(endDate(new Date()- 5*60*1000));
					$scope.begindatePublicBC = $("#begindatePublicBC").val();
					$scope.enddatePublicBC = $("#enddatePublicBC").val();
				}else{
					...
					$("#begindatePublicBC").val(startTime);
					$("#enddatePublicBC").val(endTime);
					$scope.begindatePublicBC = $("#begindatePublicBC").val();
					$scope.enddatePublicBC = $("#enddatePublicBC").val();
				}
			...
			CommonPerson.Base.LoadingPic.PartShow('loadingDivBC');//加载状态
			...
			$http({
				...
				params : {
					...
					'pageSize' : $scope.pageSizeSelect,
					'devicetype':3,
					'begindate':$scope.begindatePublicBC,
					'enddate':$scope.enddatePublicBC,
...
//	 			}
		...
		$scope.changeViewNotHistorySearchbc = function() {
			$scope.showModelDivBCHistory="${ctx}/tpl/ledgerPage/boxChange/boxChangeHistoryData.jsp";
			if(bcfirstFlagForHS == 0){
				$scope.onSelectPage(1);
				bcfirstFlagForHS++;
			}else{
				$("#box_change_result_table").show();
		...
		$scope.$on('toTabsBC', function(event,tabNum) {
			if(tabNum == 2){
				$scope.changeViewNotHistorySearchbc();
			}
		});
		...
		$scope.exportData = function(){
			...
			url +='&devicetype='+3;
			url +='&begindate='+$scope.begindatePublicBC;
			url +='&enddate='+$scope.enddatePublicBC;
...
//小时数数据
function getHoursBC($scope, $stateParams,$http){
