...
selectDevStatusLegend_All = function(legendICO){
	if($(legendICO).find('i:eq(0)').is(":visible")==true){
		...
		//设置设备
		$(".draggable").filter(".necloud_huiliuxiang,.necloud_nibianqi,.necloud_xiangbian").show()
	}else{
		...
		//设置设备
		$(".draggable").filter(".necloud_huiliuxiang,.necloud_nibianqi,.necloud_xiangbian").hide()
	}
}
...
reFlashDraggSVGDevCore2 = function(result,flag ,oldMapMap){
	...
	for(var i =0;i<narr1.length;i++){
		...
		//narr2[i] = getColor(result.inverterRtDataBatchList.comm_status[i]);
	}
