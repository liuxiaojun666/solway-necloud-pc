...
<script type="text/javascript">
	...
	app.controller('meterHistoryStatLineCtrl', function($scope, $http, toaster, $document, $stateParams) {
		...
		$scope.getHistoryPowerLineData = function() {
			...
			$http({
				...
				getPowerV(data.echarts_xaxisTime, data.echarts_predictPower, data.echarts_power, data.echarts_luminousIntensity, $http, $scope,type);
	...
	var myChart;
	function getPowerV(echarts_xaxisTime, echarts_predictPower, echarts_power, echarts_luminousIntensity, $http, $scope,type) {
		var scaleArr = [];
		for(var i=0;i<echarts_power.length;i++){
			if(Number(echarts_power[i]) != 0 && Number(echarts_luminousIntensity[i]) != 0){
				var currentProportion = (Number(echarts_power[i])/Number(echarts_luminousIntensity[i]));
			}else{
...
<div ng-controller="meterHistoryStatLineCtrl" >
