...
<script type="text/javascript">
	...
	app.controller('HistoryPowerLineCtrl', function($scope, $http, $document, toaster) {
		...
		$scope.getHistoryPowerLineData = function() {
			...
			if((stationIdArray==null||stationIdArray.length==0)&&(deviceIdArray==null||deviceIdArray.length==0)){
				toaster.pop('error','','请至少选择一台逆变器!');
				return false;
			}
			if(stationIdArray.length+deviceIdArray.length>5){
				toaster.pop('error','','逆变器最多只能查看5条功率趋势图!');
				return false;
			}
			$("#ssglqx").hide();
			...
			$http({
				...
				for (var i = 0; i < data.echarts_powers.length; i++) {
					echarts_legend.push(data.echarts_powers[i].deviceName+legname);
					invCap.push($scope.inverterCap[data.echarts_powers[i].did]);
					
				};
				
				echarts_legend.push('光照强度');
                if($scope.isChecked){
                    getNewData(data.echarts_xaxisTime, echarts_legend, data.echarts_powers, data.echarts_luminousIntensity, $http, $scope,invCap);
                }else{
                    getPowerV(data.echarts_xaxisTime, echarts_legend, data.echarts_powers, data.echarts_luminousIntensity, $http, $scope,'0');
                }
        ...
        //重组新的归一化数据 
        function getNewData(echarts_xaxisTime, echarts_legend, echarts_powers, echarts_luminousIntensity, $http, $scope,invCap){
            for(var i = 0;i<echarts_powers.length;i++){
                var newPowerArr = [];
                if(invCap[i] &&invCap[i] > 0){
                	var curentPower = echarts_powers[i].power;
                    for(var j = 0;j<curentPower.length;j++){
                        var currentProportion = 0;
                        if(Number(curentPower[j]) != 0 && Number(echarts_luminousIntensity[j]) > 25){
                            currentProportion = (Number(curentPower[j] * 1000)/(Number(echarts_luminousIntensity[j]) * invCap[i])).toFixed(2);
                        }else{
                            currentProportion = 0;
                        }
            ...
            getPowerV(echarts_xaxisTime, echarts_legend, echarts_powers, echarts_luminousIntensity, $http, $scope,'1');
        }
		//查询电站
		$scope.stationd = {};
		...
		$scope.getInverterList = function() {
			...
			.success(function(result) {
				...
				for(var i=0;i<$scope.inverterList.length;i++){
					$scope.inverterCap[$scope.inverterList[i].serialnumber] = $scope.inverterList[i].realcapacity;
	...
	function getPowerV(echarts_xaxisTime, echarts_legend, echarts_powers, echarts_luminousIntensity, $http, $scope,type) {
...
<div ng-controller="HistoryPowerLineCtrl" >
	...
	<div class="wrapper-md ng-scope">
		<div class="panel panel-default">
			<div class="row wrapper">
				<!-- <div class="col-sm-2 ">
					<div class="input-group">
						<ui-select ng-model="stationd.selected" theme="bootstrap" id="textChange" ng-change="textChange()" style="width:180px;">
							<ui-select-match placeholder="请选择电站..." ng-model="stationd.selected.stationname" >{{$select.selected.stationname}}</ui-select-match>
			...
			<div class="col-sm-3" style="line-height: 34px;"><input  type="checkbox" ng-checked="isChecked" ng-click="showScale()" style="vertical-align: sub;">逆变器归一化</div>
			</div>
			<div class="row wrapper">
				...
				<!-- <div class="col-sm-11 ">
					<div class="col-sm-2 wrapper-xs" ng-repeat="vo in selectedStationList">
						<span class="wrapper-xs body-bg-color" ng-bind="vo.stationname"></span><span class="data-red wrapper-xs a-cur-poi body-bg-color" ng-click="selectStation(false,vo.id,vo.name)">X</span>
					</div>
				</div>
				<div class="col-sm-1 m-t-xs"></div> -->
				...
				<div class="modal-dialog">
					<div class="modal-content">
						...
						<div class="modal-body" style="min-height: ;max-height: 350px;overflow-y: auto;">
							<div class="row wrapper">
								<!-- <div class="col-sm-2 ">
									<input type="checkbox" ng-click="selectStation(station.chk,stationd.selected.id,stationd.selected.stationname);" ng-model="station.chk" class="invCb"/><span ng-bind="stationd.selected.stationname"></span>
								</div> -->
								<div class="col-sm-12">
