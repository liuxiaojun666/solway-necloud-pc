...
<div ng-controller="DeviceStationCtrl">
	...
	<div div class="map-nav pos-rlt" style="background-color: #f9fafc;box-shadow: none;border-bottom: 1px solid #ddd;">
		...
		<!-- <span class="font-h3 blue-1 text-black" style="line-height: 50px;margin-left: 10px;">列表视图</span> -->
    ...
    <ul class="nav navbar-nav navbar-right m-r-xxl" name="selectListLegend">
        ...
        <a class="btn no-shadow navbar-btn shadowBtn none" ng-click="selectListLegend('shadowBtn')">
    ...
    <div class="wrapper-md ng-scope col-sm-12">
        <div class=" panel panel-default">
            ...
            <div class="col-sm-12 no-padder" id="deviceTabs">
                ...
                <tab>
                    <tab-heading>
                        <span class=" wrapper-sm white-1 a-cur-poi to-trigger" ng-click="rtStatistics(0);">箱变</span>
                    </tab-heading>
                    <div id="bBoxChangeData" data-ng-include="'${ctx}/tpl/rtMonitorPage/deviceStation/bBoxChangeData.jsp'"></div>
                </tab>
                <tab>
                    <tab-heading>
                        <span class=" wrapper-sm white-1 a-cur-poi to-trigger" ng-click="rtStatistics(1);">逆变器</span>
                    </tab-heading>
                    <div id="bInverterData" data-ng-include="'${ctx}/tpl/rtMonitorPage/deviceStation/bInverterData.jsp'"></div>
                </tab>
                <tab>
                    <tab-heading>
                        <span class=" wrapper-sm white-1 a-cur-poi to-trigger" ng-click="rtStatistics(2);">汇流箱</span>
                    </tab-heading>
                    <div id="bJunctionData" data-ng-include="'${ctx}/tpl/rtMonitorPage/deviceStation/bJunctionData.jsp'"></div>
                </tab>
                <button style="height : 37px" class=" btn btn-info pull-right" ng-click="exportRTData();" type="button">导出快照</button>
            </div>
        </div>
    </div>
</div>
...
<script type="text/javascript">
    ...
    $("#tabList").on("click","li", function() {
        ...
        switch(htmlValue){
            case "箱变": $('#deviceTabs .to-trigger').eq(0).trigger('click')
                    break;
            case "逆变器": $('#deviceTabs .to-trigger').eq(1).trigger('click')
                    break;
            case "汇流箱": $('#deviceTabs .to-trigger').eq(2).trigger('click')
                    break;
...
/*function initShowDate(data, curPage, singleNum) {
    var showData = [];
    pageNums = Math.ceil(data.length/singleNum)||pageNums;//总页数
    ...
    for (var i = 0; i < data.length; i++) {
        if (((curPage-1)*singleNum < i+1) && (i+1 <= curPage*singleNum )) {
...
function showfilterArr() {
    ...
    if ($('.shadowBtn .fa-check-circle-o').is(':visible')) {//"阴影遮挡"
        filterArr.push('05');
    }
    return filterArr;
}
/*function showfilterSearchArr(arr) {
    var searchWords = $.trim($('#searchWords').val())
    if (!searchWords) {
        return arr;
    }
    var newArr = [];
    for (var i = 0; i < arr.length; i++) {
        ...
        if (arr[i].code.indexOf(searchWords) != -1 || arr[i].name.indexOf(searchWords)!= -1) {
            newArr.push(arr[i]);
        }
    };
    return newArr;
...
app.controller('DeviceStationCtrl', function($scope, $http, $state, $stateParams) {
    ...
    $scope.getDeviceType = function(){
        $http({
                ...
                switch($scope.timerFlag){
                case "箱变": $('#deviceTabs .to-trigger').eq(0).trigger('click')
                		//此处会引起脚本错 getbBoxChangeData is not defined
                		getbBoxChangeData($scope,$http);
                        break;
                case "逆变器": $('#deviceTabs .to-trigger').eq(1).trigger('click')
               			 getbInverterDate($scope,$http);
