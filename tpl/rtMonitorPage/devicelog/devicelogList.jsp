								...
								<tr ng-repeat="vo in data ">
									...
									<td>
										<span ng-if="vo.deviceType!=4">
											<a ng-bind="vo.devicename"  ng-click="showInformation(vo.deviceId,vo.deviceSerialNnumber,vo.deviceType,vo.pstationid,vo.hasJB);" class="href-blur"></a>
										</span>
										<span ng-if="vo.deviceType==4">
									...
									<!--  <td>
										<a ng-if="vo.eventType==1||vo.eventType==2||vo.eventType==-9" class="text-info"><i class="icon-note" ng-click="createFaultHand(vo.id,vo.handStatus);" ></i></a>
									</td> -->
									<td>
...
<script type="text/javascript">
	...
	app.controller('BoxChangeCtrl', function($scope, $http, $state,$stateParams) {
    	...
    	$scope.createFaultHand = function(id,handStatus){
        	if(handStatus=='02'||handStatus=='03'){
        		$http({method:"POST",url:"${ctx}/FaultHand/getFaultAlarmById.htm",
              		params:{id:id}}).success(function (result) {
              			if(result.needToHand==1){
              				$state.go("app.taskRespMan", {
        		     				InId : result.taskId,
        		     				backUrl :4
        		     		});
              			}else{
              				$state.go("app.faultAlarmHand",{faultId:id,backUrl :4});
              			}
        			});
			} else {
				$state.go("app.faultAlarmHand",{faultId:id,backUrl :4});
    	...
    	$scope.toMessageCenter = function(id){
       		$http({method:"POST",url:"${ctx}/BaseMessage/selectPIdByBusiTypeAndBusiNo.htm",
             		params:{busiNo:id,busiType:'00'}}).success(function (result) {
             			if(result == 0){
             				...
             				//$state.go("app.faultAlarmHand",{busino:id});
             			}
       		});
		}
...
// 								$scope.station.unshift({id : "",stationname : '请选择电站'});
// 							}
...
// 								alert($scope.stationd.selected.id);
								$("#pstationid").val(angular.copy($scope.stationd.selected.id));
							}
						}); */
	...
	selectDevice = $scope.selectDevice = function(){
		...
		$http({
			...
			if($scope.device.length>0){
				$scope.device.unshift({name :$scope.device.name="请选择设备",id : $scope.device.id=""});
				if($stateParams.deviceId){
					for(var i=0;i<$scope.device.length;i++){
						if($stateParams.deviceId == $scope.device[i].id){
							$scope.deviced.selected= { name: $scope.device[i].name,serialnumber:  $scope.device[i].id};
							break;
						}
					}
				} else {
					$scope.deviced.selected= {name: $scope.device[0].name,serialnumber:  $scope.device[0].id};
				}
...
// 	$scope.dictionary = function(){
		$http({
			...
			if($scope.dictionary !=null && $scope.dictionary.length>0){
			$scope.dictionary.unshift({dictValue : -9,dictName : '通讯中断'});
			$scope.event.selected= {dictName: $scope.dictionary[0].dictName,dictValue: $scope.dictionary[0].dictValue};
			}
		})

	$scope.dictionaryChange= function () {
         $("#dictValue").val(angular.copy($scope.event.selected.dictValue));
	}
...
// 			var	dicthd=$("#dictV").val();
// 			if(dicthd==99 || dicthd==11 || dicthd==12 || dicthd==13 || dicthd==14){
// 				$("#handle").hide();
// 			}else{
// 				$("#handle").show();
// 			}
...
// 			}
			...
			$http({
				method : "POST",
				url : "${ctx}/DeviceLog/deviceRunList.htm",
				params : {
					'pageIndex' : page - 1,
			...
			//根据设备类型设置准备加载页面连接
				switch(msg.deviceType) {
					...
					case "2":
						...
						break;
					case "3":
						$scope.showHistoryData_byRTMDeviceLogId_incloud="${ctx}/tpl/ledgerPage/boxChange/boxChangeDetail.jsp";
						break;
					default:
	...
	function getServerDate2(){
		...
		$.ajax({
			type:"post",
			async:false,
			url:"${ctx}/Login/getServerDate.htm",
			success:function(msg){
				time = new Date(msg);
				return msg;
			},error : function(json) {
