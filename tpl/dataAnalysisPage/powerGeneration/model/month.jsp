...
<div class="mete-con clearfix" ng-controller="monthChartCtrl">
	<div class="pull-right" ng-show="tab == 'dataGraph'">
		<div class="col-xs-12 wrapper-xs points" style="text-align: right;padding: 15px;">
			...
			<!-- <span class="m-t-xs no-padder text-center">
				<i class="fa fa-minus" style="color: rgb(0,189,189)"></i> <span
					class="col-9 m-r-xs text-1-1x">计划发电量<small></small></span> 
			</span> -->
			<!-- <span class="m-t-xs no-padder text-center">
				<i class="fa" style="color:rgb(87,92,209);vertical-align: top;">...</i> <span
					class="col-9 m-r-xs text-1-1x">计划完成率<small></small></span> 
			</span> -->
		...
		<table  class="table  table-striped b-t b-light bg-table">
			...
			<tbody >
				<tr ng-repeat="kwh in kwhList">
					...
					<td class="text-center"><span>{{kwh.plan_kwh}}</span>
					</td>
					<td class="text-center"><span>{{kwh.finish_r}}</span></td>
...
<script>
app.controller('monthChartCtrl',function($scope, $http, $state, $stateParams) {
	...
	$scope.monthData = function(){
		$http.get("./MobileHmDeviceMonitor/getMonthChartGeneratingPower.htm",{
			params : {
