...
<script src="${ctx}/theme/js/data-format.js" type="text/javascript"></script>
	...
	function initPageData(id){
		...
		if(id != "" && id != null){
			 ...
			 $.ajax({
				type:"post",
				url:"${ctx}/Replaced/selectById.htm",
				data:{"id":id},
				success:function(msg){
					...
					$("#replacedfact").val(msg.replacedfact);
					$("#operator").val(msg.operator);
					$("#replaceddate").val(new Date(msg.replaceddate).Format("yyyy-MM-dd"));
					$("#replacedcontent").val(msg.replacedcontent);
					$("#replacedresult").val(msg.replacedresult);
					$("#devicetype").val(msg.devicetype).change(); 
// 					$("#experiimg").val(msg.experiimg);
					if(msg.experiimg != null && msg.experiimg !=""){
						var array = msg.experiimg.split(",");
						for (var i=0;i<array.length;i++){
							$("#oldFile"+i).val(array[i]);
							$("#billImg"+i).attr("src","${imgPath}/document/opDeviceReplacedPicture/"+array[i]);
						}
					}
...
app.controller('DevicereplacedCtrl',['$http','$location','$rootScope','$scope','$state','$stateParams',
...
<div ng-controller="DevicereplacedCtrl">
	<div class="wrapper-md bg-light b-b">
		<span class="font-h4 blue-1 m-n text-black">设备技术监督</span>
	</div>
	<div class="wrapper-md ng-scope">
		<div class="panel panel-default">
		<div class="row wrapper">
		<!-- form 开始 -->
		<form class="m-t-lg col-sm-12 form-horizontal ng-pristine ng-valid" action="${ctx}/Replaced/update.htm" method="post" id="editForm" name="editForm">
...
<!-- 			<input type="hidden"  name="devicetype" id="devicetype"> -->
         ...
         <div class="form-group">
             <label class="col-lg-2 control-label">改造厂商</label>
              	<div class="col-lg-3">
                	<input type="text" id="replacedfact" maxlength="50" name="replacedfact" class="form-control formData">
              	</div>
              	
            <label class="col-lg-2 control-label">改造项目</label>
              <div class="col-lg-3">
                	<input type="text" id="replacedcontent" maxlength="50" name="replacedcontent" class="form-control formData">
              </div>
          </div>  
               
      	<div class="form-group">
           <label class="col-lg-2 control-label">改造时间</label>
              <div class="col-lg-3">
                	<input type="text" id="replaceddate" name="replaceddate" class="form-control  dateTime">
              </div>
           <label class="col-lg-2 control-label">改造执行人</label>
               <div class="col-lg-3">
                   <input type="text" id="operator" maxlength="40" name="operator" class="form-control formData">
              </div>
       </div> 
               
       <div class="form-group">
           <label class="col-lg-2 control-label">改造结果</label>
              <div class="col-lg-8">
				<textarea rows="4" maxlength="500" name="replacedresult" id="replacedresult" class="form-control formData"></textarea>
			</div>
        </div>
