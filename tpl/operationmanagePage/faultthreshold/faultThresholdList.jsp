...
<script type="text/javascript">
	...
	function deleteBatch() {
		...
		if (conti) {
			var options = {
				type : "post",
				dataType : "json",
				url : "${ctx}/Threshold/deleteBatch.htm?"
	...
	app.controller('FaultThreshold', function($scope, $http, $state) {
		$scope.createInverter = function(id, name) {
			$state.go("app.addfaultThreshold", {
				InId : id
			});
		}
		$scope.showInformation = function(id) {
			$state.go("app.faultThresholdDetail", {
				InId : id
			});
		}
		...
		$scope.onSelectPage = function(page) {
			...
			$http({
				method : "POST",
				url : "${ctx}/Threshold/thresholdList.htm",
				params : {
					'pageIndex' : page - 1,
...
<div ng-controller="FaultThreshold">
	<div class="wrapper-md bg-light b-b">
	 	<span class="font-h3 blue-1 m-n text-black">预警阀值</span>
	</div>
...
<!-- 			<td ng-bind="vo.pstationid"></td> -->
			<td ><a ng-bind="vo.pstationname"  ng-click="showInformation({{ vo.id }});" style="color: blue"></a></td>
<!-- 			<td ng-bind="vo.deviceid"></td> -->
			<td ng-bind="vo.devicename"></td>
			<td ng-bind="vo.checkstartdate | date:'yyyy-MM-dd HH:mm:ss'"></td>
			<td ng-bind="vo.checkenddate | date:'yyyy-MM-dd HH:mm:ss'"></td>
			<td ng-bind="vo.averagev"></td>
			<td ng-bind="vo.deviationrange"></td>
			<td>
