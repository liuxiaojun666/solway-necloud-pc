...
<div  class="modal statModal col-sm-12 no-padder "  id="syScore" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
	<div class="modal-dialog modal-lg" style="height: auto;">
		<div class="modal-content col-sm-12 no-padder" style="height: auto;">
			<div class="modal-body no-padder"  id="text" style="overflow-y: auto; overflow-x: hidden;">
<div class="col-sm-12 no-padder panel wrapper"  ng-controller="staScoreCtrl" style="margin-bottom: 0;">
	...
	<div class="col-sm-12 no-padder">
		<div class="col-sm-12 no-padder rollDiv">
			...
			<div class="col-sm-7 wrapper" style="padding-top:0;">
				<div class="b-b-5 col-sm-12" ng-repeat="com in listData">
					<div class="col-sm-3 no-padder text-center">
						<div class="scopeCir m-n" style="height: auto;width: auto;"><span ng-bind="com.value">-</span></div>
					...
					<div class="col-sm-9">
						<p class="font-md m-t-sm m-b-xs" ng-bind="com.describeStr"></p>
					</div>
				</div>
			</div>
		</div>
...
<script>
	...
	app.controller('staScoreCtrl',function($scope, $http, $state,$rootScope) {
		$scope.$on("dateChangeCtrl", function (event, currentPsId) {
			$scope.getAllData($scope.mapTimeDay, currentPsId);
		});
		...
		$scope.getAllData=function(nowData, currentPsId){
			if(!!currentPsId){
				$scope.stationListId = currentPsId;
			}
			console.log("$scope.stationListId:"+$scope.stationListId);
			console.log("$scope.mapTimeDay:"+$scope.mapTimeDay);
			//标题的时间
	...
	function powerAnalyze(tab1,tab2,tab3){
	   ...
	   require(
			[ 'echarts', 'echarts/chart/radar', 'echarts/chart/line'], function(echarts) {
