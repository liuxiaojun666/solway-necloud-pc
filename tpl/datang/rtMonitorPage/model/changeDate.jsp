		...
		<div class="pull-left no-padder">
			...
			<a class="no-padder" ng-show="dataType == 'day'">
				...
				<span class="input-append date form_datetime" id="changeTimeIdTimer-day" data-link-field="date-day">
					...
					<span id="showDate-day" class="showdate m-l m-r font14 time-color" style="vertical-align: super;" ng-bind="mapTimeDay | date:'yyyy-MM-dd'"></span>
				</span>
...
app.controller('overviewHeaderCtrl',function($scope, $http, $state, $stateParams) {
	...
	$scope.$on('chageDateInit',function (e, obj) {
		...
		$scope.dailyJt = obj.endDate.Format("yyyy-MM-dd") == $scope.mapTimeDay.Format("yyyy-MM-dd")
		$("#date-day").val($scope.mapTimeDay.Format("yyyy-MM-dd"))
	})

	init();//初始化页面
	function init(){
		$scope.dataType = "day";//默认显示月的数据
	}
