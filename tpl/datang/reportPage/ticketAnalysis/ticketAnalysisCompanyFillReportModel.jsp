...
<div  ng-controller="dtTicketAnalysisCompanyFillModelCtrl" class="page-con">
			...
			<table class="table border-green table-striped">
				...
				<tbody class="font12">
					<tr ng-repeat="item in formData">
						...
						<td class="text-center">{{item.ticketOutNum}}</td>
						<td class="text-center">{{item.ticketInNum}}</td>
...
<script>
app.controller('dtTicketAnalysisCompanyFillModelCtrl',function($scope, $http, $state, $stateParams) {
	...
	$scope.ticketNames = [{"ticketName":"电气一种工作票","ticketType":108},{"ticketName":"电气二种工作票","ticketType":109},{"ticketName":"电力线路一种工作票","ticketType":110},{"ticketName":"电力线路二种工作票","ticketType":111},
						{"ticketName":"电气倒闸操作票","ticketType":115},{"ticketName":"断电保护措施票","ticketType":112},{"ticketName":"动火工作票","ticketType":113},{"ticketName":"动土工作票","ticketType":114}];
	...
	$scope.saveEditRow = function(item,index,type){
		...
		$http({
            ...
            data : {
            	id:item.id,
            	analysisId :$scope.id,
            	busiDate:(new Date($scope.dateStr)).Format("yyyy-MM"),
            	content:content,
            	analysisDataType:type
            }
		 }).success(function(result) {
	...
	function initDate(ParamTime){
		...
		$("#changeTimeId-stopTime").on('hide',function(ev) {
			...
			$scope.judgeSelectTime(stringTime,function(value){
				...
				$scope.dateStr = new Date(timestamp).Format("yyyy-MM");
				$("#showStopTime").text(new Date(timestamp).Format("yyyy-MM"));
				if(!value){
					alert("请选择未填写过的时间");
					getTicketExecuteNumData();// 选完时间，获取数据
				}else{
					 $scope.formData = [];
				}
			});
