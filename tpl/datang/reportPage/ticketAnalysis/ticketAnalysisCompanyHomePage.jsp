...
<div ng-controller="dtTicketAnalysisCompanyHomePageCtrl">
	<div class="page-padding word-black">
		...
		<div class="clearfix">
			<div class="list-page-con background-white" ng-show="listOrFill == 'listPage'">
				<paging>
					<div class="col-sm-12 font16" style="padding: 10px 0 !important;height:50px;">
						<div class="col-sm-2 no-padder">     
						    <button class="btn btn-sm btn-info m-r-8" ng-click="newFillPage()"> 新增</button>  
					...
					<table id="result_table" class="table table-striped b-t b-light">
						...
						<tbody class="font12">
							<tr ng-repeat="vo in data">
								...
								<td ng-bind="vo.busiDate | date:'yyyy-MM-dd'"></td>
								<td class="text-center" ng-bind="vo.companyName"></td>
								<td class="text-center" ng-bind="vo.createDate | date:'yyyy-MM-dd'"></td>
								<td  class="text-right">
			...
			<div class="fill-page-con" ng-show="listOrFill == 'fillPage'">
				<div data-ng-include="'${ctx}/tpl/datang/reportPage/ticketAnalysis/ticketAnalysisCompanyFillReportModel.jsp'"></div> 
			</div>
		</div>
	</div>
</div>
...
app.controller('dtTicketAnalysisCompanyHomePageCtrl', function($scope,$rootScope,$http, $state, $stateParams,$interval) {
	...
	$scope.onSelectPage = function(page) {
		...
		$http({
	 		method : "POST",
	 		url : "${ctx}/rpmTicketAnalysis/page.htm",
	 		params : {
				pageIndex : page - 1,
	...
	$scope.showFillPage = function(item){
		...
		$scope.$broadcast("fillItem",item);
	}
	
	$scope.newFillPage = function(){
		$scope.listOrFill = "fillPage";
	...
	$scope.deleteRow = function(id){
		if(confirm("确定要删除这条数据吗？")){
			$http({
				method : "POST",
				url: "${ctx}/rpmTicketAnalysis/delete.htm",
	...
	$scope.deleteBatch = function(){
		...
		if (conti) {
			...
			$http({
				method : "POST",
				url : "${ctx}/rpmTicketAnalysis/deleteBatch.htm",
				params : {
					'ids' :checkedIds
				}
			}).success(function(result) {
