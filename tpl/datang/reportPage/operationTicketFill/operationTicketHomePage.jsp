...
<div ng-controller="dtOperationTicketHomePageCtrl">
	...
	<div class="page-padding word-black">
		...
		<div class="clearfix">
					...
					<table id="result_table" class="table table-striped b-t b-light">
						<thead style="background: #9fe8dd;">
							<tr>
								...
								<th  class="text-center">操作票票号</th>
								<th class="text-center">操作开始时间</th>
								<th class="text-center">操作结束时间</th>
						...
						<tbody class="font12">
							<tr ng-repeat="vo in data">
								...
								<td class="text-center" ng-bind="vo.ticketNo"></td>
								<td class="text-center" ng-bind="vo.startTime|date:'yyyy-MM-dd'"></td>
								<td class="text-center" ng-bind="vo.endTime|date:'yyyy-MM-dd'"></td>
								<td class="text-left" ng-bind="vo.operationUser"></td>
			...
			<div class="fill-page-con" ng-show="listOrFill == 'fillPage'">
				<div data-ng-include="'${ctx}/tpl/datang/reportPage/operationTicketFill/operationTicketFillReportModel.jsp'"></div> 
			</div>
		</div>
	</div>
</div>
...
app.controller('dtOperationTicketHomePageCtrl', function($scope,$rootScope,$http, $state, $stateParams,$interval) {
	...
	$scope.onSelectPage = function(page) {
		...
		$http({
	 		method : "POST",
	 		url : "${ctx}/rpdOperateTicket/page.htm",
	 		params : {
	...
	$scope.showFillPage = function(item){
		$scope.listOrFill = "fillPage";
		$scope.$broadcast("fillItem",[item,null]);
	}
    ...
    $scope.deleteRow = function(id){
    	if(confirm("确定要删除这条数据吗？")){
    		$http({
    			method : "POST",
    			url : "${ctx}/rpdOperateTicket/delete.htm",
    			params : {
    				'id' :id,
    			}
    		}).success(function(result) {
	...
	$scope.deleteBatch = function(){
		...
		if (conti) {
			...
			$http({
				method : "POST",
				url : "${ctx}/rpdOperateTicket/deleteBatch.htm",
				params : {
					'ids' :checkedIds,
				}
			}).success(function(result) {
