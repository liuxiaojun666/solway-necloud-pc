...
<div class="" ng-controller="benchmarkingDayCtrl">
    <div class="b-b benchmarkingHead wrapper-md">
        <ul class="list-inline">
            ...
            <li class="active"><a class="white-1 a-cur-poi ng-scope" href="#/app/DTBenchmarkingWeek">周</a></li>
        ...
        <div>
            <div ng-repeat="(date, list) in data">
                <table class="workBriefingTable">
                    <thead>
                        <tr>
                            <th colspan="7" class="text-right">日期：{{ date }}——{{ weekEndDate(date) }}</th>
                        </tr>
...
app.controller('benchmarkingDayCtrl', function($scope, $http, $state,$stateParams) {
    ...
    $scope.weekEndDate = function (dateStr) {
        var resultDate = getDateForStringDate(dateStr)
        resultDate.setDate(resultDate.getDate() + 6)
        return new Date(resultDate).Format("yyyy-MM-dd")
    }

    $scope.mapTimeDay = new Date
    $scope.mapTimeDay.setDate($scope.mapTimeDay.getDate() - 7)
