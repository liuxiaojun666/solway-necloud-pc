...
<div ng-controller="dtWorkTicketHomePageCtrl">
	...
	<div class="page-padding word-black">
		...
		<div class="clearfix">
					...
					<table id="result_table" class="table table-striped b-t b-light">
						<thead style="background: #9fe8dd;">
							<tr>
								...
								<th class="text-center">接票时间</th>
								<th class="text-center">许可时间</th>
								<th class="text-left">工作认可人</th>
						...
						<tbody class="font12">
							<tr ng-repeat="vo in data">
								...
								<td class="text-center" ng-bind="vo.acceptTime|date:'yyyy-MM-dd'"></td>
								<td class="text-center" ng-bind="vo.allowTime|date:'yyyy-MM-dd'"></td>
								<td class="text-left" ng-bind="vo.allowUser1"></td>
			...
			<div class="fill-page-con" ng-show="listOrFill == 'fillPage'">
				<div data-ng-include="'${ctx}/tpl/datang/reportPage/workTicketFill/workTicketFillReportModel.jsp'"></div>
			</div>
		</div>
	</div>
</div>
...
app.controller('dtWorkTicketHomePageCtrl', function($scope,$rootScope,$http, $state, $stateParams,$interval) {
    ...
    $scope.deleteRow = function(id){
    	if(confirm("确定要删除这条数据吗？")){
    		$http({
    			method : "POST",
    			url : "${ctx}/rpdWorkTicket/delete.htm",
    			params : {
    				'id' :id,
    			}
    		}).success(function(result) {
	...
	$scope.deleteBatch = function(){
		...
		if (conti) {
			...
			$http({
				method : "POST",
				url : "${ctx}/rpdWorkTicket/deleteBatch.htm",
				params : {
					'ids' :checkedIds,
				}
			}).success(function(result) {
