...
<script type="text/javascript">
	...
	app.controller('WpHistoryStatLineCtrl', function($scope, $http, toaster, $document, $stateParams) {
		...
		$scope.getWpHistoryPowerLineData = function () {
			...
			$http({
				method : "POST",
				url : "${ctx}/WpHistoryPowerLine/getWpHistoryPowerLineData.htm",
				params : {
				...
				/**data.echarts_xaxisTime = ['01:01','01:02','01:03','01:04','01:05','01:06','01:07']
				data.echarts_power= [1,2,3,4,5,6,7]
				data.echarts_predictPower = []
				data.echarts_windSpeed = []*/
				if(!data.echarts_xaxisTime){
					data.echarts_xaxisTime = [];
				}
				if(!data.echarts_predictPower){
					data.echarts_predictPower = [];
				}
				if(!data.echarts_power){
					data.echarts_power = [];
				}
				...
				getPowerV(data.echarts_xaxisTime, data.echarts_predictPower, data.echarts_power, data.echarts_windSpeed, $http, $scope,type);
	...
	function getPowerV(echarts_xaxisTime, echarts_predictPower, echarts_power, echarts_windSpeed, $http, $scope,type) {
		/**var scaleArr = [];
		for(var i=0;i<echarts_power.length;i++){
			if(Number(echarts_power[i]) != 0 && Number(echarts_windSpeed[i]) != 0){
				var currentProportion = (Number(echarts_power[i])/Number(echarts_windSpeed[i]));
			}else{
...
<div ng-controller="WpHistoryStatLineCtrl" >
