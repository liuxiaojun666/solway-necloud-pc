$(function () {
        ...
        for (var i=0,ii=data.length;i<ii;i++){
            ...
            if (Object.prototype.toString.call($obj) == "[object Array]"){
                for (var j=0,jj=$obj.length;j<jj;j++){
                    ...
                    $obj[j].find("rect").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj[j]):data[i].status]);
                    $obj[j].find("path").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj[j]):data[i].status]);
                    $obj[j].find("line").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj[j]):data[i].status]);
                }
            }else {
                if (!!callBack && !callBack($obj, data[i])) continue;
                //$obj.find("rect").attr("fill", this.statusColor[!!callBackStatus?callBackStatus($obj):data[i].status]);
                $obj.find("rect").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data[i].status]);
                $obj.find("path").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data[i].status]);
                $obj.find("line").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data[i].status]);
            }
        }
    };
    ...
    Xb.prototype.flushXb = function (data, callBackStatus, callBack) {
        ...
        for (var i=0,ii=data.length;i<ii;i++){
            ...
            if (Object.prototype.toString.call($obj) == "[object Array]"){
                for (var j=0,jj=$obj.length;j<jj;j++){
                    if (!!callBack && !callBack($obj[j], data[i])) continue;
                    $obj[j].find("stroke").attr("fill", this.statusColor[!!callBackStatus?callBackStatus($obj[j]):data[i].status]);
                    $obj[j].find("ellipse").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj[j]):data[i].status]);
                }
            }else {
                if (!!callBack && !callBack($obj, data[i])) continue;
                $obj.find("rect").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data[i].status]);
                $obj.find("ellipse").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data[i].status]);
            }
        }
    };
    ...
    NbqHlx.prototype = getCommon();
    NbqHlx.prototype.collectionNbqHlx = function () {
        if (this.collectionFlag){
            var nbqhlx = this._svgView.find("g[type='NBQHLX']");
            var nbqhlxOld = null, deviceId = null;
            for (var i=0,ii=nbqhlx.length;i<ii;i++){
                deviceId = nbqhlx.eq(i).attr("deviceId");
                nbqhlxOld = this.nbqhlxs['nbqhlx_' + deviceId];
                if (!!nbqhlxOld){
                    if (Object.prototype.toString.call(nbqhlxOld) != "[object Array]"){
                        var o = nbqhlxOld;
                        nbqhlxOld = [];
                        nbqhlxOld.push(o);
                        this.nbqhlxs['nbqhlx_' + deviceId] = nbqhlxOld;
                    }
                    nbqhlxOld.push(nbqhlx.eq(i));
                }else {
                    this.nbqhlxs['nbqhlx_' + nbqhlx.eq(i).attr("deviceId")] = nbqhlx.eq(i);
                }
            }
            this.nbqhlxs['len'] = nbqhlx.length + 1;
            this.collectionFlag = false;
            this.flushRemoveSvgTitle();
        }
    };
    NbqHlx.prototype.createNbqHlxBunchNumber = function (data) {
        var len; _this = this;
        if (data == null || (len = data.length) == 0) return;
        if (!_this.bunchFlag) return;
        var _iThis, bunch, d, bLen;
        for(var i=0;i<len;i++){
            d = data[i];
            if ((bunch = d.bunch) == null || (bLen = bunch.length) == 0) continue;
            _iThis = _this.nbqhlxs['nbqhlx_' + d.deviceId];
            if(Object.prototype.toString.call(_iThis) == "[object Array]"){
                for(var j=0,jj=_iThis.length;j<jj;j++){
                    _this.dealSingleNbqHlxBunch(_iThis[j], bunch);
                }
                continue;
            }
            _this.dealSingleNbqHlxBunch(_iThis, bunch);
        }
        _this.bunchFlag = false;
    };
    NbqHlx.prototype.dealSingleNbqHlxBunch = function (_this, bunch) {
        var $rects, bid;
        $rects = _this.find("rect");
        $rects.attr("rect-flag", "0");
        $rects.eq(0).removeAttr("rect-flag");
        ...
        for (var j=0,jj=bunch.length;j<jj;j++){
            $rects.eq((bid = bunch[j].bid) * 2).attr("bid-status", bid).removeAttr("rect-flag");
            $rects.eq(bid * 2 + 1).attr("bid-shadow", bid).removeAttr("rect-flag");
            //$rects.eq(parseInt(bunch[j].bid, 10) + 1).attr("device-bid", bunch[j].bid).removeAttr("rect-flag");
        }
        _this.find("rect[rect-flag='0']").remove();
    };
    NbqHlx.prototype.changeNbqHlxStatus = function ($obj, data, callBackStatus) {
        var $rect = $obj.find("rect:eq(0)");
        data = !!callBackStatus?callBackStatus($rect, data):data;

        $obj.find("rect").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data.status]);
        $obj.find("path").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data.status]);
        $obj.find("line").attr("stroke", this.statusColor[!!callBackStatus?callBackStatus($obj):data.status]);

        var bunck;
        for (var k=0,kk=data.bunch.length;k<kk;k++){
            bunck = !!callBackStatus?callBackStatus($obj, data.bunch[k]):data.bunch[k];
            //$obj.find("rect[device-bid='" + data.bunch[k].bid + "']").attr("fill", this.statusBunchColor[bunck.status]).attr("stroke", this.shadowColor[bunck.shadow]);
            $obj.find("rect[bid-status='" + data.bunch[k].bid + "']").attr("fill", this.statusBunchColor[bunck.status]);
            $obj.find("rect[bid-status='" + data.bunch[k].bid + "']").attr("stroke", this.statusBunchColor[bunck.status]);
            $obj.find("rect[bid-shadow='" + data.bunch[k].bid + "']").attr("fill", this.shadowColor[bunck.shadow]);
            $obj.find("rect[bid-shadow='" + data.bunch[k].bid + "']").attr("stroke", 'none');
        }
    };
    NbqHlx.prototype.flushNbqHlx = function (nbqhlx, callBackStatus, callBack) {
        // alert("this.collectionFlag:" + this.collectionFlag);
        var data = nbqhlx.data;
        this.collectionNbqHlx();
        this.createNbqHlxBunchNumber(data);
        var $obj = null;
        for (var i=0,ii=data.length;i<ii;i++){
            $obj = this.nbqhlxs["nbqhlx_" + data[i].deviceId];
            if (Object.prototype.toString.call($obj) == "[object Array]"){
                for (var j=0,jj=$obj.length;j<jj;j++){
                    if (!!callBack && !callBack($obj[j], data[i])) continue;
                    this.changeNbqHlxStatus($obj[j], data[i], callBackStatus);
                }
            }else {
                if (!!callBack && !callBack($obj, data[i])) continue;
                this.changeNbqHlxStatus($obj, data[i], callBackStatus);
            }
        }
    };
