...
app.controller('monitoringCtrl', function($scope, $rootScope, $http, $state, $stateParams, $document) {
	...
	/*$scope.showRealPower=function(flag){
		switch (parseInt(flag)){
			case 1:
				...
				$scope.showPowerIndexData="tpl/rtMonitorPage/monitor/modal/showWeatherPowerModal.jsp";
				break;
			case 2:
				$('#powerIndexModal').modal();
				if ($scope.dailyJt){
					$scope.showPowerIndexData="tpl/rtMonitorPage/monitor/app_bInverterData.jsp";
				}else {
			...
			case 3:
				$('#powerIndexModal').modal();
				// $scope.showPowerIndexData="tpl/reportPage/monthly/powerEffic.jsp";
				$scope.showPowerIndexData="tpl/rtMonitorPage/monitor/modal/staMonModal.jsp";
				break;
			case 4:
				$('#powerIndexModal').modal();
				$scope.showPowerIndexData="tpl/rtMonitorPage/monitor/modal/staYearModal.jsp";
				break;
			default:
				break;
		}
	...
	$("#changeTimeIdTimer").on('hide',
		function(ev) {
			$scope.changeDateByUser();
		});
	
	//重新选择时间
	$scope.changeDateByUser=function(){
		// 获取某个时间格式的时间戳
	...
	$scope.dateLeft = function() {
		...
		$("#dtp_input2").val(new Date($scope.mapTimeDay).Format("yyyy-MM-dd hh:mm:ss"));
		$scope.dailyJt = (new Date($scope.mapTimeDay).Format("yyyy-MM-dd")) == (new Date($scope.today).Format("yyyy-MM-dd"))
	...
	$scope.getstaDayData=function(){
		$http({
			method : "POST",
			url : "./MobileRtmDeviceMonitor/getRunTimeMonitor.htm",
			timeout: 5000,
			params : {
				'dateString':new Date($scope.mapTimeDay).Format("yyyy-MM-dd"),
				'powerStationId':null //$scope.powerStationId
			}
		}).success(function (msg) {
...
// 实时功率
function getPowerVForRT(echarts_xaxisTime,echarts_predictPower,echarts_power,echarts_luminousIntensity,$http, $scope) {
	myChart = echarts.init(document.getElementById('ssglqx'))			
	window.addEventListener("resize", function() {
					myChart.resize();
				});
				...
				timer2=setInterval(function(){
					$http.get("./MobileHmDeviceMonitor/getDayChartRealTimePower.htm",
							{params : {},timeout: 5000
							}).success(function(response) {
								option.series[2].data = response.echarts_luminousIntensity;
...
function getActPwrLineData2($http, $scope) {
	console.log($scope.mapTimeDay +"==$scope.mapTimeDay" + new Date($scope.mapTimeDay).Format("yyyy-MM-dd"));
	$http.get(
			"./MobileHmDeviceMonitor/getDayChartRealTimePower.htm",
			{
				...
				if(response.echarts_xaxisTime==null){
					...
					getPowerVForRT(response.echarts_xaxisTime,response.echarts_predictPower,response.echarts_power,response.echarts_luminousIntensity,$http, $scope);
				}
	}).error(function(response) {
	});
}
